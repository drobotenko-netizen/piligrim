...
export default function HoursClient() {
  ...
  async function load() {
    setLoading(true)
    try {
      const r = await fetch(`${API_BASE}/api/iiko/sales/hours?date=${date}`, { cache: 'no-store', headers: { 'x-role': 'ADMIN' } })
            ...
            <tbody>
              {rows.map((r: any, i: number) => (
                <tr key={i} className="border-t">
                  <td className="px-2 py-1">{r.hour}</td>
                  <td className="px-2 py-1">{r.net}</td>
                </tr>
              ))}
              {!rows.length && <tr><td className="px-2 py-3 text-sm text-muted-foreground" colSpan={2}>Нет данных на дату</td></tr>}
            </tbody>
          </table>
        )}
      </div>
