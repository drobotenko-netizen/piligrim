...
export function createAdjustmentsRouter(prisma: PrismaClient) {
  ...
  // GET / - список корректировок по месяцу
  ...
  // POST / - создать корректировку
  router.post('/', asyncHandler(async (req: Request, res: Response) => {
    if (!(prisma as any).adjustment) {
      return res.status(503).json({ 
        error: 'adjustments model not available (run prisma migrate/generate)' 
      })
    }
    
    const { employeeId, date, kind, amount, reason } = req.body || {}
    
    if (!employeeId || !date || !kind || !Number.isFinite(amount)) {
      return res.status(400).json({ error: 'bad request' })
    }
    
    if (!['bonus', 'fine', 'deduction'].includes(kind)) {
  ...
  // PATCH /:id - обновить корректировку
  router.patch('/:id', validateId(), asyncHandler(async (req: Request, res: Response) => {
    ...
    if (typeof body.kind === 'string') {
      if (!['bonus', 'fine', 'deduction'].includes(body.kind)) {
        return res.status(400).json({ error: 'invalid kind' })
      }
    ...
    if (body.amount !== undefined) {
      patch.amount = Number.isFinite(body.amount) 
        ? Math.trunc(Math.abs(body.amount)) 
        : undefined
    }
  ...
  // DELETE /:id - удалить корректировку
