...
export function createAdminUsersRouter(prisma: PrismaClient) {
  ...
  router.post('/', requirePermission(prisma, 'users.manage'), asyncHandler(async (req: Request, res: Response) => {
    const tenant = await getTenant(prisma, req as any)
    ...
    const created = await prisma.user.create({ 
      data: { tenantId: tenant.id, fullName: data.fullName, phone: data.phone } 
    })
  ...
  router.patch('/:id', validateId(), requirePermission(prisma, 'users.manage'), asyncHandler(async (req: Request, res: Response) => {
  ...
  router.post('/:id/roles', validateId(), requirePermission(prisma, 'users.manage'), asyncHandler(async (req: Request, res: Response) => {
    const tenant = await getTenant(prisma, req as any)
    const id = req.params.id
