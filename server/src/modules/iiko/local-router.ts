...
export function createIikoLocalRouter(deps: { buildDayRangeIso: (d: string) => { from: string; to: string } }) {
  ...
  router.get('/sales/hours/matrix', asyncHandler(async (req: Request, res: Response) => {
  ...
  router.get('/sales/revenue/month', asyncHandler(async (req: Request, res: Response) => {
    const prisma = (req as any).prisma || req.app.get('prisma')
    if (!prisma) return res.status(503).json({ error: 'prisma not available' })
  ...
  // Monthly returns
  router.get('/sales/returns/month', asyncHandler(async (req: Request, res: Response) => {
    const prisma = (req as any).prisma || req.app.get('prisma')
    if (!prisma) return res.status(503).json({ error: 'prisma not available' })
    const year = Number(String(req.query.year || ''))
    const month = Number(String(req.query.month || ''))
  ...
  // Monthly deleted
  router.get('/sales/deleted/month', asyncHandler(async (req: Request, res: Response) => {
    const prisma = (req as any).prisma || req.app.get('prisma')
    if (!prisma) return res.status(503).json({ error: 'prisma not available' })
    const year = Number(String(req.query.year || ''))
    const month = Number(String(req.query.month || ''))
  ...
  router.get('/sales/total/month', asyncHandler(async (req: Request, res: Response) => {
    const prisma = (req as any).prisma || req.app.get('prisma')
    if (!prisma) return res.status(503).json({ error: 'prisma not available' })
    const year = Number(String(req.query.year || ''))
    const month = Number(String(req.query.month || ''))
  ...
  router.get('/sales/available-months', asyncHandler(async (req: Request, res: Response) => {
    const prisma = (req as any).prisma || req.app.get('prisma')
    if (!prisma) return res.status(503).json({ error: 'prisma not available' })
  ...
  router.get('/import/status', asyncHandler(async (req: Request, res: Response) => {
    const prisma = (req as any).prisma || req.app.get('prisma')
    if (!prisma) return res.status(503).json({ error: 'prisma not available' })
    const year = Number(String(req.query.year || ''))
    if (!Number.isInteger(year) || year < 2000 || year > 2100) return res.status(400).json({ error: 'year invalid' })
